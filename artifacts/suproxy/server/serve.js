...
function serveLandingPage(req, res, template, appName) {
  ...
  const html = template.replace(/BASE_URL_PLACEHOLDER/g, baseUrl).replace(/EXPS_URL_PLACEHOLDER/g, host).replace(/APP_NAME_PLACEHOLDER/g, appName);
...
server.listen(port, "0.0.0.0", () => console.log(`Serving static Expo build on port ${port}`));
